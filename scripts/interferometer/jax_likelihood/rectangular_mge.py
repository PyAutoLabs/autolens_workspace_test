...
our data. In this example we fit a model where:
 ...
 - The lens galaxy has an MGE bulge and `Isothermal` mass; the `ExternalShear` is an `al.MassField`
   in `fields=`.
...
assert (
    mass_indices
), "interferometer/rectangular_mge: no mass parameters found for sensitivity check"
...
# the second JIT compile within memory (this script's Path A uses
# batch_size=6, which together with a second compiled vmap can OOM the
# default 16 GB box).
import gc
