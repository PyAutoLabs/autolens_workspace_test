"""
Potential Correction: Subhalo Recovery Validation (Iterative LM)
================================================================

The iterative half of the `imaging/subhalo_recovery.py` split (#267): the warm-started, gauge-constrained
Levenberg-Marquardt confirmation leg of the gravitational-imaging (potential correction) validation. It repeats
the one-shot setup (simulate, arc mask, smooth model, pixelizations) and the one-shot `FitDpsiSrcImaging` fit —
needed to build the warm-start `x0`, and asserted here as a cheap sanity check — then runs the iterative engine
and asserts the recovered convergence correction still localizes the true subhalo. The LM leg is what confirms
the one-shot correction is a genuine cost minimum rather than a point the solve drifts off (a stationarity
check); the one-shot script alone cannot show that.

This script is deliberately NOT listed in `smoke_tests.txt`. The weekly `workspace-smoke.yml` and the
`release-integrate` channel run every script in the repo, so it is covered there automatically; only the
one-shot leg (`imaging/subhalo_recovery.py`) stays on the per-PR gate, where the iterative leg was ~80% of the
runtime while reproducing the one-shot dkappa metrics to 3 decimal places.

The one-shot script's `iterative Laplace log evidence` print is deliberately not carried over: it was measured
at ~18s on its own and nothing asserts on it.
...
print("potential_correction iterative subhalo recovery checks passed")
