"""
Path A PoC: jit-wrap ``analysis.fit_from`` for Delaunay source + MGE lens
=========================================================================

Intersection of two already-shipped siblings:

- ``delaunay_pytree.py`` — Hilbert image-mesh + Delaunay pixelization source,
  AdaptSplit regularization, narrow single-entry ``galaxy_image_plane_mesh_grid_dict``
  fallback shipped in PyAutoGalaxy#361.
- ``mge_pytree.py`` — ``Basis``-of-Gaussians (MGE) light side.

Task prompt (``admin_jammy/prompt/issued/fit_imaging_pytree_delaunay_mge.md``):
...
"""
__Dataset__ — same on-disk dataset used by ``delaunay_mge.py``.
"""
sub_size = 4
...
"""
__Model__ — MGE bulge + Isothermal + ExternalShear lens, Delaunay pixelization source.
"""
# Lens: MGE bulge + Isothermal + ExternalShear
